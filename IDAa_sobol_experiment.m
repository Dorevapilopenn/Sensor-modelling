...
%   Output JSON: IDAa_sobol_optimal.json
%
% Example smoke run:
...
fprintf('IDAa Sobol result written to %s\n', cfg.outputJson);
end
...
cfg.outputJson = 'IDAa_sobol_optimal.json';
...
cfg.workers = 6;
cfg.algorithmVersion = 'IDAa_sobol_fast_pls_reuse_svm_squared_hinge_scalar_ida_v1';
...
PN = double(scaled(:, 7) >= 0.5);
knobs = array2table([scaled(:, 1:6), PN, scaled(:, 8:9)], ...
    'VariableNames', {'meanHD', 'deltaHD', 'deltaD1', 'deltaD2', 'deltaK1', 'deltaK2', 'PN', 'logCs', 'r'});

K1 = scaled(:, 1) - scaled(:, 2) / 2;
K4 = scaled(:, 1) + scaled(:, 2) / 2;
K2 = K1 + scaled(:, 3);
K3 = K2 + scaled(:, 5);
K5 = K4 + scaled(:, 4);
K6 = K5 + scaled(:, 6);
isN = PN == 0;
K5(isN) = K4(isN) + scaled(isN, 4) + scaled(isN, 6);
K6(isN) = K4(isN) + scaled(isN, 4);
constants = [K1, K2, K3, K4, K5, K6];

sampleInfo = table();
sampleInfo.Cs = 10 .^ scaled(:, 8);
sampleInfo.C0 = sampleInfo.Cs .* (10 .^ scaled(:, 9));
end
...
result.sobolVariables = {'meanHD', 'deltaHD', 'deltaD1', 'deltaD2', 'deltaK1', 'deltaK2', 'PN', 'logCs', 'r'};
