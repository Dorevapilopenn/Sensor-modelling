...
nsamp=50;
c_0     = [16.5e-6 0.0001];            % tot conc in initial solution Bi,Cl
ncomp   = length(c_0);          % number of components

C_tot_H=c_0(1,1)*ones(nsamp,1);
C_tot_D=c_0(1,2)*rand(nsamp,1);
C_tot=[C_tot_H C_tot_D];

c_comp_guess = [1e-10 1e-10 ];  % initial guess for [H],[G] and [D]
for i=1:nsamp
...
for i=1:nsamp
    C(i,:)=NewtonRaphson(Model,beta,C_tot(i,:),c_comp_guess,i);
    c_comp_guess=C(i,1:ncomp);  % new guess = previous comp conc
end
C_colored=C(:,find(absorbing));         %leave only absorbing D, HD and GD species
...
save D:/Sens1_S.mat D_meas1 D_meas2