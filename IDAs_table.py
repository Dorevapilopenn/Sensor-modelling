...
# Helper functions
def safe_pearsonr(vals1, vals2):
    """Compute Pearson correlation, returning 0 if either input is constant."""
    if np.nanstd(vals1) == 0 or np.nanstd(vals2) == 0:
        return 0
    try:
...
# Load datasets
df_U = pd.read_csv("IDAs_res9U.csv", header=None)
# Assign column names per confirmation
cols = ["K1","K2","K3", "HD", "deltaD","deltaK","Et","Class1","Class2"]
df_U.columns = cols
...
for key, df in datasets.items():
    ...
    p3 = os.path.join(out_dir, f"{key}_Et_deltad_deltak.png"); make_heatmap_mean(df, "deltaK","deltaD","Et", p3)
    results[key] = [table_path, p1, p2, p3]

    # Non-interactive stratified correlation: deltaD & deltaK, corr(Et, deltaD)
    strat_corr_to_file(df, "deltaD", "deltaK", "Et", "HD", os.path.join(out_dir, f"stratcorr_deltaD_deltaK_Et_vs_HD.png"))
