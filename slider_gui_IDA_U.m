function param = slider_gui_IDA_U
    ...
    if isfile('slider_values.mat')
        saved_data = load('slider_values.mat', 'param_values');
        param_values = saved_data.param_values;
    else
        % Set default values if no saved values exist
        for i = 1:num_params
            if i <= 6
                param_values(i) = mean(range_0_10);
            elseif contains(param_names{i}, 'lambda')
                param_values(i) = mean(range_400_700);
            elseif i <= 12
                param_values(i) = mean(range_40k_100k);
            elseif i <= 15
                param_values(i) = mean(range_50_100);
            end
        end
    end
    ...
    function runSimulation()
        ...
        save('slider_values.mat', 'param_values'); % Save values to file
